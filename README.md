...
<p align="center">
  ...
  macOS dev utilities, runtime debugging, icon generation, diagramming, localization, and system diagnostics — bundled into a single plugin with auto-updates.
</p>
...
| 🗑️ | `mac-app-uninstall` | Browse every installed app, then uninstall any of them **with every leftover it left behind** — an auditable stand-in for AppCleaner / CleanMyMac. Tiers each finding into safe / review / shared-do-not-touch, covers launchd jobs, privileged helpers and system extensions, and moves everything to the Trash. Never runs `sudo`, never runs `rm`. |
